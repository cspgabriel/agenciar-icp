...
import { Database, Search, Filter, Loader2, ArrowRight, Activity, Cpu } from 'lucide-react';
...
function App() {
  ...
  async function fetchLeads() {
    ...
    setError(null);
    try {
      // Busca todos os leads, focando nos que vieram do Raio-X
      ...
      // Filtrar apenas leads que tenham dados de raiox (ou icp se já convertido)
      const raioxLeads = (data || []).filter(l => l.raiox_data || l.icp_data);
      setLeads(raioxLeads);
    } catch (err: any) {
      console.error(err);
  ...
  const handleICPSaved = (leadId: string, newIcp: any) => {
    ...
    if (selectedLead?.id === leadId) {
      setSelectedLead({ ...selectedLead, icp_data: newIcp });
    }
  };
  ...
  return (
    <div className="min-h-screen relative p-4 lg:p-8">
      ...
      <div className="fixed inset-0 z-[-1] overflow-hidden pointer-events-none bg-slate-50">
        <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full bg-purple-300 blur-[120px] opacity-40 mix-blend-multiply"></div>
        <div className="absolute top-[20%] right-[-10%] w-[50%] h-[50%] rounded-full bg-indigo-300 blur-[120px] opacity-40 mix-blend-multiply"></div>
      </div>
      ...
      <header className="max-w-7xl mx-auto mb-10 flex items-center justify-between animate-fade-in">
         ...
         {selectedLead && (
           <div className="hidden lg:flex px-4 py-2 glass-card rounded-full items-center gap-2 text-sm font-semibold text-[var(--brand-accent)]">
             <Activity size={16}/> Visualizando ICP Ativo
           </div>
         )}
      </header>

      {selectedLead ? (
        <LeadDetail lead={selectedLead} onBack={() => handleSelectLead(null)} onICPSaved={handleICPSaved} />
      ) : (
        <main className="max-w-7xl mx-auto animate-fade-in">
          
          <div className="glass-card rounded-2xl p-6 mb-8 flex flex-col md:flex-row gap-4 justify-between items-center z-10 relative">
             <div className="relative w-full md:w-96">
                <Search size={20} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                <input 
                  type="text" 
                  placeholder="Buscar lead ou empresa..." 
                  className="w-full bg-white/60 border border-slate-200 rounded-xl py-3 pl-12 pr-4 outline-none focus:ring-2 focus:ring-[var(--brand-accent)] focus:border-transparent transition-all"
                  value={searchTerm}
             ...
             <div className="flex w-full md:w-auto gap-4">
                <div className="relative w-full md:w-auto">
                  ...
                  <select 
                    className="w-full md:w-auto appearance-none bg-white/60 border border-slate-200 rounded-xl py-3 pl-12 pr-10 outline-none focus:ring-2 focus:ring-[var(--brand-accent)] font-medium text-slate-700"
                    value={filterScore}
          ...
          {error && (
            <div className="bg-red-50 text-red-600 p-4 rounded-xl border border-red-200 mb-8 font-medium shadow-sm">
              {error}
            </div>
          )}
          ...
          {loading ? (
             <div className="flex flex-col items-center justify-center p-20 gap-4">
               ...
               <p className="text-slate-500 font-medium animate-pulse-slow">Sincronizando com Supabase...</p>
             </div>
             ...
             <div className="text-center p-20 glass-card rounded-3xl border-dashed border-2 border-slate-300">
               ...
               <p className="text-slate-500 max-w-sm mx-auto mt-2">Os leads do formulário Raio-X aparecerão aqui assim que forem preenchidos.</p>
             </div>
          ) : (
               ...
               {filteredLeads.map(lead => {
                 ...
                 >
                   <div>
                     ...
                     <div className="text-slate-600 text-sm space-y-1 mb-6">
                       <p className="truncate">📧 {lead.email}</p>
                       <p className="truncate">📱 {lead.phone}</p>
                       <p className="text-xs text-slate-400 mt-2">
                         Captado em {format(new Date(lead.created_at), "dd 'de' MMM, yyyy 'às' HH:mm", { locale: ptBR })}
                       </p>
                     </div>
                   </div>
                   ...
                   <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                     <span className={`text-xs px-3 py-1.5 rounded-full font-bold ${
                       ...
                       {hasICP ? '⭐ ICP Perfilado' : '⏳ Aguardando IA'}
                     </span>
                     <ArrowRight size={20} className="text-slate-300 group-hover:text-[var(--brand-accent)] transform group-hover:translate-x-1 transition-all" />
                   </div>
                 </div>
               )})}
             </div>
